...
$$
    ...
    BEGIN
        ...
        SELECT count(*) FROM br7own.error_list
            WHERE annotated_msg = 'to resolve' INTO errors_to_move;
        RAISE NOTICE 'Errors to resolve: %', errors_to_move;

        SELECT count(*) FROM br7own.error_list AS e, br7own.error_list_triggers AS t
            WHERE e.annotated_msg = 'to resolve' AND e.error_id = t.error_id INTO triggers_to_move;
        RAISE NOTICE 'Triggers to resolve: %', triggers_to_move;

        SELECT count(*) FROM br7own.error_list AS e, br7own.error_list_notes AS n
            WHERE e.annotated_msg = 'to resolve' AND e.error_id = n.error_id INTO notes_to_move;
        RAISE NOTICE 'Notes to resolve: %', notes_to_move;

        -- Archive the records
        SELECT br7own.archive_resolved_records(1) INTO function_output;
        RAISE NOTICE 'Function output: %', function_output;
        ASSERT function_output = ARRAY[1], 'Function output was incorrect, expected "[1]"';

        SELECT br7own.archive_resolved_records(4) INTO function_output;
        RAISE NOTICE 'Function output: %', function_output;
        ASSERT function_output = ARRAY[12, 13, 14], 'Function output was incorrect, expected "[12, 13, 14]"';

        -- Check the correct number of records have been archived
        SELECT count(*) FROM br7own.archive_error_list WHERE annotated_msg = 'to resolve' INTO errors_moved;
        RAISE NOTICE 'Errors moved: %', errors_moved;
        SELECT count(*) FROM br7own.archive_error_list AS e, br7own.archive_error_list_triggers AS t WHERE e.annotated_msg = 'to resolve' and e.error_id = t.error_id  INTO triggers_moved;
        RAISE NOTICE 'Triggers moved: %', triggers_moved;
        SELECT count(*) FROM br7own.archive_error_list AS e, br7own.archive_error_list_notes AS n WHERE e.annotated_msg = 'to resolve' and e.error_id = n.error_id INTO notes_moved;
        ...
        ASSERT (SELECT count(*) FROM br7own.archive_log as a JOIN br7own.archive_error_list as e ON a.log_id = e.archive_log_id WHERE a.archived_by = 'Resolved record archiver') = 4,
            'Archived errors do not all relate to the log';

        -- Check we've marked the log as being created by this function
        ASSERT (SELECT archived_by FROM br7own.archive_log limit 1) = 'Resolved record archiver',
        ...
        PERFORM br7own.archive_resolved_records(1000);
        ASSERT (SELECT COUNT(*) FROM br7own.archive_log) = 2,
