...
To improve Bichard we are continually asking more complex questions of our data. Remodelling the data is required to reduce the manual time/effort needed, or to make some questions answerable at all e.g. a breakdown of total cases entering Bichard with certain result codes, by code and by month, for 2026. Depending on latency, it's possible that this dataset could be used to self-serve answers to common questions to stakeholders (i.e. dashboards), and to help with the next generation of in-system reporting.
