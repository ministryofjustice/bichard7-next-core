...
import createAuditLogEvent from "./lib/auditLog/createAuditLogEvent"
...
export default async (
  ...
  try {
    ...
    hearingOutcome = await enrichAho(hearingOutcome, pncGateway, events)

    events.push(getIncomingMessageLog(hearingOutcome.AnnotatedHearingOutcome.HearingOutcome, message, messageType))
    ...
    if (isIgnored) {
      ...
      if (hearingOutcome.Exceptions.length > 0) {
        events.push(getExceptionsGeneratedLog(hearingOutcome))
      }
      if (triggers.length > 0) {
        events.push(getTriggersGeneratedLog(triggers, hearingOutcome.Exceptions.length > 0))
      }
    ...
    events.push(
      createAuditLogEvent("message-rejected", "error", "Message Rejected by CoreHandler", "CoreHandler", {
