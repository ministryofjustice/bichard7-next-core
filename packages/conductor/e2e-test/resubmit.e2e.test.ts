...
const setupCase = async (messageId: string, phase: number = 1): Promise<CaseRow> => {
  const ahoXml = fs
  ...
  const [caseRow] = (await db`
      ...
      VALUES
        (${ahoXml}, ${ahoXml}, '01ZD0304208', ${new Date()}, 2, 'HO100304', 0, ${messageId}, ${new Date()}, '001', ${phase}, 0, 2, 0, 3, 'user_name')
      RETURNING *
    `) satisfies CaseRow[]
...
describe("resubmit", () => {
  ...
  it("successfully runs the resubmit workflow", async () => {
    ...
    await startWorkflow("resubmit", { messageId }, messageId)
    await waitForCompletedWorkflow(messageId, undefined, undefined, "resubmit")
  ...
  it("triggers the bichard_phase_2 successfully", async () => {
    ...
    await startWorkflow("resubmit", { messageId }, messageId)
    const result = await waitForCompletedWorkflow(messageId, undefined, undefined, "bichard_phase_2")
  ...
  it("has Audit Logs", async () => {
    ...
    await startWorkflow("resubmit", { messageId }, messageId)
    await waitForCompletedWorkflow(messageId, undefined, undefined, "bichard_phase_1")
