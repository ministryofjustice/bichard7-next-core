...
describe("ReportResults", () => {
  it("renders nothing when reportType is undefined", () => {
    cy.mount(<ReportResults isStreaming={false} hasRun={false} reportType={undefined} rows={[]} config={null} />)
    cy.get(".results-area").should("not.exist")
  })

  it("renders the loading state when isStreaming is true", () => {
    cy.mount(<ReportResults isStreaming={true} hasRun={false} reportType={dummyReportType} rows={[]} config={null} />)
    cy.get(".results-area").should("have.attr", "aria-busy", "true")
    cy.contains(/Loading.*report\.\.\./i).should("be.visible")
  })

  it("renders the no results message when hasRun is true but rows are empty", () => {
    cy.mount(
      <ReportResults isStreaming={false} hasRun={true} reportType={dummyReportType} rows={[]} config={mockConfig} />
    )
  ...
  it("renders the no results message when config is null despite having rows", () => {
    cy.mount(
      <ReportResults isStreaming={false} hasRun={true} reportType={dummyReportType} rows={mockRows} config={null} />
    )
    cy.get("output").contains("No results found for the selected criteria.").should("be.visible")
  })

  it("renders the ReportTable when hasRun is true, rows exist, and config is provided", () => {
    cy.mount(
      <ReportResults
        isStreaming={false}
        hasRun={true}
