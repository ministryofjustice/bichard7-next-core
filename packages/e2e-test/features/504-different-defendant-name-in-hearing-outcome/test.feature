Feature: {504} 504-different-defendant-name-in-hearing-outcome
			...
			"""
			MadeTech Definition:
			Ensure that LEDS is updated when an incoming court message contains different defendant name for the same case, provided the ASN matches. In this test, LEDS contains the defendant name William Brown, while the incoming court message contains the Paul Smith. The expected behaviour is that Bichard successfully updates LEDS despite the name difference.
			"""
