---
layout: default
title: Help
---

Contact the Bichard team for support at [moj-bichard7@madetech.com](mailto:moj-bichard7@madetech.com)

## What's New
  - [Improvements to Case List filtering](whats-new/case-list-filtering/)
    <!-- - [Reports](whats-new/reports/) -->
    <!-- - [User Management](whats-new/user-management/) -->

## User Login
  - [Logging in to Bichard](user-login/logging-in-to-bichard/)
  - [Resetting your password](user-login/resetting-password/)
  - [Changing your password](user-login/changing-your-password/)
  - [Setting up your account](user-login/new-account-setup/)

## Roles
- [Bichard Roles](bichard-functionality/bichard-roles)

### Exceptions Handler
- [Exceptions](bichard-functionality/exceptions/)
- [Exception resolution](bichard-functionality/exceptions/resolution.html)

### Trigger Handler
- [Triggers](bichard-functionality/triggers/)
- [List of all triggers](bichard-functionality/triggers/list/)

### User Managers
  - [View details of users in your Force](user-management/viewing-users/)
  - [Edit the details of a user](user-management/editing-users/)
  - [Create a new user](user-management/creating-users/)
  - [Remove a user](user-management/removing-users/)

### Supervisors
  - [Supervisors](supervisors/)

## How to use this system
- [Useful Tips](bichard-functionality/useful-tips/)
- [Police National Computer (PNC)](bichard-functionality/how-bichard-updates-pnc/)
- [Bichard functionality](bichard-functionality/)
- [Glossary](glossary/)
