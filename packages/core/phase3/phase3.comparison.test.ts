...
import type { DbRecords, Phase3E2eComparison } from "../comparison/types/ComparisonFile"
...
const checkDatabaseMatches = async (expected: any): Promise<void> => {
  ...
  const expectedError = expected.errorList[expected.errorList.length - 1]
  if (expectedError) {
    // expect(errorList[0]).toStrictEqual(expectedError)
    ...
    expect(errorList[0].user_updated_flag).toEqual(expectedError.user_updated_flag)
...
describe("phase3", () => {
  ...
  describe.each(tests)("should correctly process $file", (comparison: Phase3E2eComparison) => {
    ...
    it("should generate the correct PNC operations", () => {
      normalisePncOperations(comparison.pncOperations)
      ...
      expect(actualPncOperations).toStrictEqual(comparison.pncOperations)
    })
