...
describe("getPncCourtCode", () => {
  ...
  it("Should check that the 'thirdLevelPsaCode' isNaN", () => {
    ...
    const pncCourtCode = getPncCourtCode(ouCodes, courtHouseCode)

    expect(pncCourtCode).toBeInstanceOf(Error)
    expect((pncCourtCode as Error).message).toBe("PSA code is not a number")
  })
