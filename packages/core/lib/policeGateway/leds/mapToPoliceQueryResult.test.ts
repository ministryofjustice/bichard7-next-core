...
import ledsAsnQueryResponseWithEmptyDisposals from "../../../tests/fixtures/leds-asn-query-response-002.json"
import mapToPoliceQueryResult from "./mapToPoliceQueryResult"

describe("mapToPoliceQueryResult", () => {
  it("maps LEDS ASN query response to police query result", () => {
  ...
  it("sets offence disposals to undefined when disposalResults is empty", () => {
    const policeQueryResult = mapToPoliceQueryResult(
      ledsAsnQueryResponseWithEmptyDisposals as AsnQueryResponse,
      "dummy checkname"
    )

    expect(policeQueryResult.courtCases?.[0].offences[0].disposals).toBeUndefined()
  })
})
