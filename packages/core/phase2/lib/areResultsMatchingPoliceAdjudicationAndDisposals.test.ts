...
describe("areResultsMatchingPoliceAdjudicationAndDisposals", () => {
  it("returns true when no results", () => {
    ...
    const result = areResultsMatchingPoliceAdjudicationAndDisposals(aho, offence)
  ...
  it("returns false when no police court cases", () => {
  ...
  it("returns true when results match adjudication and disposals", () => {
  ...
  it("returns false when results match adjudication but not disposals", () => {
  ...
  it("returns false when results match disposals but not adjudication", () => {
    const aho = generateAhoMatchingPncAdjudicationAndDisposals({})
    const offence = aho.AnnotatedHearingOutcome.HearingOutcome.Case.HearingDefendant.Offence[0]
  ...
  it("checks for exceptions when an offence index and function is provided", () => {
    ...
    areResultsMatchingPoliceAdjudicationAndDisposals(aho, offence, 0, checkExceptionFn)
