...
import { getEmailAddressCookieConfig } from "../../src/useCases/getEmailAddressCookieConfig"
...
describe("getEmailAddressFromCookie", () => {
  ...
  it.each(emailAddressCookieTypes)("should return null if cookie value is expired (%s)", (cookieType) => {
    ...
    const request = { cookies: {} } as { cookies: NextApiRequestCookies }
    request.cookies[cookieName] = "1629887818347|dummy@dummy.com.SATxSgt/eVyzah6Uig09ooiyW/YW+G4XRX/EgAQ2yDo"
  ...
  it.each(emailAddressCookieTypes)("should return null if cookie has invalid signature (%s)", (cookieType) => {
    const { cookieName } = getEmailAddressCookieConfig(config, cookieType)
    const request = { cookies: {} } as { cookies: NextApiRequestCookies }
    request.cookies[cookieName] = "1629887818347|dummy@dummy.com.InvalidSignature"
