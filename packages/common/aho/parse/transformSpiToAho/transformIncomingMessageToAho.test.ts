...
describe("transformIncomingMessageToAho", () => {
  it("should transform the incoming message to an AHO", () => {
    const inputMessage = String(readFileSync(path.resolve(__dirname, "../fixtures/input-message-routedata-001.xml")))
  ...
  it("should decode '&' xml entities in the incoming message correctly", () => {
    const inputMessage = String(
      readFileSync(path.resolve(__dirname, "../fixtures/input-message-routedata-entities.xml"))
    )
  ...
  it("should decode double escaped '&amp;lt;' xml entities in the incoming message correctly", () => {
    const inputMessage = String(
      readFileSync(path.resolve(__dirname, "../fixtures/input-message-routedata-double-entities.xml"))
    )
