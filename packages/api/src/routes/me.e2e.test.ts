...
describe("/me e2e", () => {
  ...
  it("will return the current user with a correct JWT", async () => {
    const [encodedJwt, user] = await createUserAndJwtToken(helper.dataSourceGateway)
  ...
  it("will return the current user that has user groups, with a correct JWT", async () => {
    ...
    const [encodedJwt, user] = await createUserAndJwtToken(helper.dataSourceGateway, expectedGroups)
  ...
  it("will throw an error with a user that has no groups", async () => {
    ...
    await expect(createUserAndJwtToken(helper.dataSourceGateway, noGroups)).rejects.toThrow("User has no Groups")
  })
})
