...
describe("GET /v1/users/:userId", () => {
  ...
  it("returns 200 OK with retrieved user", async () => {
    ...
    const response = await app.inject({
      ...
      method: "GET",
      url: V1.User.replace(":userId", "123")
    })

    expect(response.statusCode).toBe(OK)
    const body = await response.json()

    expect(body).toEqual(expect.objectContaining({ username: "username01" }))
  })
  ...
  it("returns 403 Forbidden for unauthorised user", async () => {
    ...
    const response = await app.inject({
      ...
      method: "GET",
      url: V1.User.replace(":userId", "456")
    })
