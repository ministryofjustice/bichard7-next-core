...
describe("jwtVerify", () => {
  ...
  it("will return an error if the username does not exists in the db", async () => {
    ...
    await expect(jwtVerify(gateway, jwtString)).rejects.toThrow("User UserNotFound does not exist")
  })
  ...
  it("will return a User if the username exists in the db and the JWT ID matches", async () => {
    ...
    const result: User | undefined = await jwtVerify(gateway, jwtString)
