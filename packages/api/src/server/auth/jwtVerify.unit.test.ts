...
describe("jwtVerify", () => {
  ...
  it("will return an error if the username does not exists in the db", async () => {
    ...
    await expect(jwtVerify(fakeDataStore, jwtString)).rejects.toThrow("User UserNotFound does not exist")
  })
  ...
  it("will return a User if the username exists in the db and the JWT ID matches", async () => {
    ...
    const result: undefined | User = await jwtVerify(fakeDataStore, jwtString)
  ...
  it("will throw an error if the JWT is not valid", async () => {
    await expect(jwtVerify(fakeDataStore, "NotARealJWT")).rejects.toThrow("jwt malformed")
  })
