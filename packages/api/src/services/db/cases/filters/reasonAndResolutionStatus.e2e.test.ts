...
const insertTestCaseWithTriggersAndExceptions = async (helper: SetupAppEnd2EndHelper, args: CreateReasonCaseProps) => {
...
const createReasonCase = async (helper: SetupAppEnd2EndHelper, courtCode: string, args: CreateReasonCaseProps) => {
  ...
  await createCase(helper.postgres, {
    ...
    triggerInsertedAt: args.trigger?.triggerResolvedBy ? new Date() : null,
...
describe("Filter cases by resolution status", () => {
  ...
  beforeAll(async () => {
    ...
    await Promise.all(
      ...
      ].map((caseDetails) => insertTestCaseWithTriggersAndExceptions(helper, caseDetails))
    )
  })
  ...
  describe("Filter cases having by resolution status, reason code and user permission", () => {
    it.each([
      ...
      //   user: () => supervisor
      // }
    ])("$description", async ({ description, expectedCases, filters, user }) => {
      const { cases } = (await fetchCasesAndFilter(
        helper.postgres.readonly,
        { ...filters, ...defaultQuery },
        user()
      )) as CaseIndexMetadata

      const sortedActualCases = cases.map((c) => c.defendantName).sort()
      const sortedExpectedCases = expectedCases.sort()
      if (sortedActualCases.length !== sortedExpectedCases.length) {
        console.log(description, "============", sortedActualCases, "=======", sortedExpectedCases)
      }

      expect(sortedActualCases).toEqual(sortedExpectedCases)
    })
  })
})
