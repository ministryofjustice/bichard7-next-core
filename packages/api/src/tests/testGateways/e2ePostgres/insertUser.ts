...
export default async (sql: postgres.Sql, partialUser: Partial<FullUserRow>) => {
  if (!(partialUser.email && partialUser.username)) {
    throw new Error("Missing required attributes")
  }

  const user = partialUser as unknown as FullUserRow

  const [result]: [FullUserRow?] = await sql`
