...
export const convertAuditCaseToDto = (auditCase: AuditCase): AuditCaseDto => ({
  asn: auditCase.asn,
  courtDate: auditCase.court_date ? format(auditCase.court_date, "yyyy-MM-dd") : null,
  ...
  messageReceivedTimestamp: auditCase.msg_received_ts?.toISOString() ?? null,
  noteCount: auditCase.notes?.length,
  notes: auditCase.notes ? auditCase.notes.map(convertNoteToDto) : [],
  ptiurn: auditCase.ptiurn,
  resolutionTimestamp: auditCase.resolution_ts?.toISOString() ?? null,
