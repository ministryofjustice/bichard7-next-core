...
jest.mock("../../../services/db/cases/locking/lockTrigger", () => {
  const originalModule = jest.requireActual("../../../services/db/cases/locking/lockTrigger")
  return {
    __esModule: true,
...
describe("lockTriggers", () => {
  ...
  it("locks the triggers and generate an audit log event when user has permission to handle triggers", async () => {
    ...
    await testDatabaseGateway.writable.transaction(async (transaction) => {
      await lockTriggers(transaction, user, caseObj.errorId, auditLogEvents)
    })
  ...
  it("doesn't update audit logs when trigger locking fails", async () => {
    ...
    const mockedLockTrigger = jest.mocked(lockTrigger)
    mockedLockTrigger.mockResolvedValueOnce(Error("Dummy Trigger Error"))
