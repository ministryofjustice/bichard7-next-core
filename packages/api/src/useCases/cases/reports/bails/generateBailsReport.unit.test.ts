...
import { generateBailsReport } from "./generateBailsReport"

jest.mock("../utils/createReportAuditLog", () => ({
  createReportAuditLog: jest.fn()
}))

jest.mock("../../../../services/db/cases/reports/bails", () => ({
  bailsReport: jest.fn()
}))

describe("generateBailsReport", () => {
  let mockDatabase: DatabaseGateway
  let mockAuditLogGateway: AuditLogDynamoGateway
  let mockUser: User
  let mockQuery: BailsReportQuery
  ...
  it("should successfully run the report, capture the total count, and call createReportAuditLog", async () => {
    ...
    const result = await generateBailsReport(mockDatabase, mockAuditLogGateway, mockUser, mockQuery, mockReply)
