...
ALL_VERSIONS=$(echo $VERSIONS | jq -r ".[] | { version: ., parsedVersion: (. | split(\".\")) } | .parsedVersion[] |= tonumber | select((.parsedVersion[0] * 10000) + (.parsedVersion[1] * 100) + .parsedVersion[2] >= $FROM_VERSION) | { c: (\" bichard7-next-data-\" + .version + \"@npm:@moj-bichard7-developers/bichard7-next-data@\" + .version) } | .c")
